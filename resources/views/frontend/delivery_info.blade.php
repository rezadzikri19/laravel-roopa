                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="col-8 d-flex flex-wrap">
                                                @foreach (Session::get('cart') as $key2 => $items)
                                                    @if ($items['owner_id']==$key)
                                                    @php
                                                        $items = $items[0];
                                                    @endphp
                                                        @foreach ($items as $key3 => $item)
                                                            <div class="mb-4 col-4">
                                                                <h6 class="fw-600">{{ $item[0]["name"] }}</h6>
                                                                @php
                                                                    $item = $item[0]['costs'];
                                                                @endphp
                                                                @foreach ($item as $ongkir)
                                                                    <div class="mb-2 form-check">
                                                                        <input class="form-check-input" type="radio" name={{ $key }}{{ $key2 }} id="ongkir" value={{ $ongkir["cost"][0]["value"] }}>
                                                                        <label class="form-check-label" for="inlineRadio1">{{ $ongkir["service"] }}</label>
                                                                        <label class="form-check-label" for="inlineRadio1">(Rp {{ number_format($ongkir["cost"][0]["value"], 0, ',', '.') }})</label>
                                                                    </div>                                                    
                                                                @endforeach
                                                            </div>
                                                        @endforeach
