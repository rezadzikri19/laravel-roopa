...
class CheckoutController extends Controller
{
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        foreach (Session::get('cart') as $key => $cartItem) {
            ...
            $ongkir = $ongkir->setOrigin($data['city_id'])->setDestination($cartItem['owner_city_id'])->setWeight(1000)->setCouriers(['jne', 'tiki', 'pos'])->cost()->get();
            if (!isset($cartItem[0])) {
                array_push($cartItem, $ongkir);
        ...
        // return view('frontend.delivery_info', compact('total'));
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        // original
            ...
            $cart = $cart->map(function ($object, $key) use ($request) {
                $req = $request->all();
                if (\App\Product::find($object['id'])->user_id == $request->owner_id) {
                    $object['shipping'] = $req[$request->owner_id.$key];
                } else {
                    $object['shipping'] = 0;
                }
                return $object;
